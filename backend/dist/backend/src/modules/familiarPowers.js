...
exports.applyFamiliarPowers = exports.getStartingFamiliarPowers = void 0;
/**
 ...
 */
function getStartingFamiliarPowers() {
    // Shuffle and take 2
...
exports.getStartingFamiliarPowers = getStartingFamiliarPowers;
/**
 * Applies a single familiar power effect for this turn.
 */
function applyPower(player, state, power) {
    switch (power.effect.type) {
        ...
        case 'gainRenownOnQuest':
            ...
            const newQuestRumors = state.rumors.filter(r => r.source === 'quest' && r.id.includes(player.id));
 ...
 */
function applyFamiliarPowers(player, state) {
    for (const power of player.familiarPowers || []) {
        applyPower(player, state, power);
    }
}
exports.applyFamiliarPowers = applyFamiliarPowers;
