...
const blank:Record<CropType,number> = { mushroom:0, flower:0, herb:0, fruit:0 };

brewRouter.post("/brew", (req,res) => {
  const { recipeId, seed, moves } = req.body as BrewMatch3Result;
  const meta = (recipes as any)[recipeId];
  ...
  db.prepare("INSERT INTO potions(playerId,name,tier,quality) VALUES (?,?,?,?)")
